...
function ncdf_gewex::read_l2b_data, file, node=node, variables = variables, found = found
	...
	if ~self.clara2 then fileID = ncdf_open(file[0])
	...
	foreach prd, vars.keys() do begin
		...
		if self.clara2 then begin
			dum_file = file_search(strjoin(strsplit(file,strupcase(self.clara_default_var),/ext,/regex),pname),count=count_file)
		...
		if varID ne -1 then begin
			...
			if self.clara2 then NCDF_CLOSE, fileID
		endif
	endforeach
