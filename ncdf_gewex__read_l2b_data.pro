function ncdf_gewex::read_l2b_data, file, node=node, variables = variables, found = found
	...
	cmsaf  = self.clara2 or self.hector
	out    = hash()

	fileID = ncdf_open(file[0])
	...
	foreach prd, vars.keys() do begin
		...
		if cmsaf then begin
			ncdf_close, fileID
			dum_file = file_search(strjoin(strsplit(file,strupcase(self.CMSAF_PATH),/ext,/regex),pname),count=count_file)
		...
		if varID ne -1 then begin
			...
			if ( prd eq 'ILLUM' and strupcase(vn) eq 'SUNZEN' and cmsaf ) then begin
